...
class Users_model extends User_model
{
	 ...
	 *
	 *@access  public
	 *@return $fullname string, user fullname string
	 */
	 ...
	 */
	public function save_settings(array $settings, $user=null)
	{
		$this->load->model('user_settings_model', 'user_settings');
	...
	{	
		...
		if(array_key_exists('accepts_terms', $account) && $account['accepts_terms']==='yes')
		{
			...
			if(false === $result['error'])
			{
				...
				if(false === $result['error'])
				{
					$user=array_merge($user, array('id'=>$user_id));
	...
	public function create_session()
	{
		$this->session->set_userdata(array(
			'is_signed_in'=>true,
			'is_admin'=>(($this->type=='admin') ? true : false),
			'uid'=>$this->id,
			'email'=>$this->email,
	...
	public function login_account(array $user)
	{
		...
		if($user)
		{
			...
			else
			{
				return array(
					'error'=>true,
					'message'=>'Error. Account not activated. Contact administrator'
				);
			}
		}
	 ...
	 *@return array, results
	 */
	public function login_with(array $user)
	{
		$user = array_pop($this->where($user)->results());
	 ...
	 *@return mixed, value of session variable
	 */
	public function current_user($session_var, $value=null)
	{
		if(null!=$value) return $this->session->set_userdata($session_var, $value);
		return $this->session->userdata($session_var);
	}
	...
	{
		if(is_numeric($params)) $params=array('id'=>(int)$params);
	 ...
	 *get_leaderboard
	 *
	 *Gets accounts points leaderboard
	 *
	 *@access public
	 *@return $results array, accounts arranged in descending order of points
	 */
	public function get_leaderboard()
	{
		...
		{
			$user_points=(int)$user->get_info('points');
			$sorted_count=count($sorted_users);

			if(!empty($sorted_users))
			{
				$sorted_users[]=$user;
				$sorted_count=count($sorted_users);
				$user_index=($sorted_count-1);

				for($i=0; $l=count($sorted_users), $i<$l; $i++)
				{
					$tmp_user=$sorted_users[$i];
					$tmp_user_points=(int)$tmp_user->get_info('points');

					if($tmp_user_points < $user_points)
					{
					   $sorted_users[$i]=$user;
					   $sorted_users[$user_index]=$tmp_user;
					}
				}
			}
			else
		...
		return array('results'=>$sorted_users);
	}

	/**
	 *get_user_with_setting
	 *
	 *Gets particular users record by search params
	 *
	 *@access public
	 ...
	 *@return $results mixed, user model instances or empty arrays
	 */
	public function get_user_with_setting($setting, $value)
	{
		$this->load->model('user_settings_model', 'user_settings');
		...
		if($setting) return $this->get_user($setting->users->id);
